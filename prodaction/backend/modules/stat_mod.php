...
@$mysqlQuery = "INSERT INTO " . DB_LOG_TABLE . " (`remote_addr`, `useragent`, `client_country`, `client_build`, `client_version`, `api_version`, `device_info`, `device_id`" .
		", `os_version`, `os_id`, `action`, `from`, `where`, `weight`, `volume`, `insurancePrice`, `compSite`, `compName`, `client_language`, `apikey`) VALUES " .
		"(\"" . $mysqlHandle->real_escape_string(inet_pton($_stat_client_ipaddr)). "\", \"" . 
		$mysqlHandle->real_escape_string($_stat_client_useragent) . "\", \"" . $mysqlHandle->real_escape_string($_stat_client_country) . "\", \"" .
		$mysqlHandle->real_escape_string($_stat_client_build) . "\", \"" . $mysqlHandle->real_escape_string($_stat_client_ver) . "\", \"" . intval($_stat_api_ver) . "\", \"" .
		$mysqlHandle->real_escape_string($_stat_device_info) . "\", \"" . $mysqlHandle->real_escape_string($_stat_device_id) . "\", \"" . 
		$mysqlHandle->real_escape_string($_stat_os_version) . "\", \"" . $mysqlHandle->real_escape_string($_stat_os_id) . "\", \"" . 
		$mysqlHandle->real_escape_string($_stat_action) . "\", \"" . $mysqlHandle->real_escape_string($cityFrom) . "\", \"" . $mysqlHandle->real_escape_string($cityTo) . "\" " .
		", \"" . $weight . "\", \"" . $volume . "\", \"" . $insurancePrice . "\", \"" . $mysqlHandle->real_escape_string($csite) . "\"" .
		", \"" . $mysqlHandle->real_escape_string($cname) . "\", \"" . $mysqlHandle->real_escape_string($_stat_client_language) . "\"" .
